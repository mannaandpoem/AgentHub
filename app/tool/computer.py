...
class ComputerTool(BaseTool):
    ...
    name: str = "computer"
    description: str = """Control the computer by simulating keyboard and mouse input. Features include:
    - Mouse actions: Move cursor, click (left/right/middle/double), drag
    - Keyboard input: Type text or send key combinations
    - Screen interaction: Take screenshots and get cursor position
    - Smooth mouse movement with natural easing
    - Automatic display scaling for reliability
    All coordinates are automatically scaled to the target display resolution."""
    ...
    parameters: dict = {
        ...
        "properties": {
            "action": {
                ...
                - key: Send keyboard shortcuts or special keys (e.g., 'ctrl+c', 'enter')
                - type: Type text naturally with small delays between keystrokes
                - mouse_move: Move cursor smoothly to coordinates with natural easing
                - left_click: Perform a left mouse click (at current position)
                - left_click_drag: Click and drag from current position to target coordinates
                - right_click: Perform a right mouse click (at current position)
                - middle_click: Perform a middle mouse click (at current position)
                - double_click: Perform a double left click (at current position)
                - screenshot: Capture current screen state
                - cursor_position: Get current cursor coordinates""",
            },
            "text": {
                "type": "string",
                "description": """Text input for 'key' and 'type' actions:
                - For 'type': The exact text to type
                - For 'key': Key combination (e.g., 'ctrl+c', 'shift+tab', 'enter')
                Note: For Mac, 'super' is automatically converted to 'command'""",
            },
            "coordinate": {
                "type": "array",
                "items": {"type": "integer"},
                "minItems": 2,
                "maxItems": 2,
                "description": """[x, y] coordinates for mouse movement actions:
                - Required for 'mouse_move' and 'left_click_drag'
                - Coordinates are automatically scaled to display resolution
